		...
		if (isSliding) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mCurrentView != null) {
					...
					// 设置popupWindow的动画
					if (location[1] > 100 && location[1] < getHeight()) {
						mPopupWindow.setAnimationStyle(R.style.popwindow_delete_btn_anim_style);
						mPopupWindow.update();
						mPopupWindow
								.showAtLocation(mCurrentView, Gravity.LEFT | Gravity.TOP,
										location[0] + mCurrentView.getWidth(), location[1]
												+ mCurrentView.getHeight() / 2 - mPopupWindowHeight
												/ 2);
						// 设置删除按钮的回调
